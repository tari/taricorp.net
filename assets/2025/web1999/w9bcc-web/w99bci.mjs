...
var Module = typeof w99bci != 'undefined' ? w99bci : {};
...
var wasmBinaryFile;
if (Module['locateFile']) {
  wasmBinaryFile = 'w99bci.wasm';
  if (!isDataURI(wasmBinaryFile)) {
    wasmBinaryFile = locateFile(wasmBinaryFile);
  }
} else {
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  wasmBinaryFile = new URL('w99bci.wasm', import.meta.url).toString();
}
...
var ASM_CONSTS = {
  16396: function() {throw new Error('Bytecode must end with OP_EOF, but did not')},  
 16460: function($0) {throw new Error('Bytecode command ' + $0 + ' is not implemented')}
};
function draw_char_internal(bitmapHandle,ctxHandle,x,y,fgOpaque,fgRed,fgGreen,fgBlue,bgOpaque,bgRed,bgGreen,bgBlue){ const bitmap = Emval.toValue(bitmapHandle); const charHeight = bitmap.length; const charWidth = bitmap[0].length; const ctx = Emval.toValue(ctxHandle); const data = ctx.getImageData(x, y, charWidth + 2, charHeight); for (let y = 0; y < charHeight; y++) { for (let x = 0; x < charWidth; x++) { const b = ((y * data.width) + x) * 4; const d = bitmap[y][x]; if (d && fgOpaque) { data.data[b] = fgRed; data.data[b + 1] = fgBlue; data.data[b + 2] = fgGreen; data.data[b + 3] = 255; } else if (!d && bgOpaque) { data.data[b] = bgRed; data.data[b + 1] = bgGreen; data.data[b + 2] = bgBlue; data.data[b + 3] = 255; } } } if (bgOpaque) { for (let y = 0; y < charHeight; y++) { for (let x = charWidth; x < data.width; x++) { const b = ((y * data.width) + x) * 4; data.data[b] = bgRed; data.data[b + 1] = bgGreen; data.data[b + 2] = bgBlue; data.data[b + 3] = 255; } } } ctx.putImageData(data, x, y); return charWidth + 2; }
function draw_sprite(ctxHandle,paletteP,xc,yc,data){ const width = HEAPU8[data]; const height = HEAPU8[data + 1]; const palette = HEAPU8.subarray(paletteP, paletteP + (3 * 256)); const ctx = Emval.toValue(ctxHandle); const pixels = HEAPU8.subarray(data + 2, data + 2 + (width * height)); const canvasData = new ImageData(width, height); for (let y = 0; y < height; y++) { const row = pixels.subarray(y * width, y * width + width); for (let x = 0; x < width; x++) { const base = y * width * 4 + x * 4; canvasData.data[base + 0] = palette[3 * row[x]]; canvasData.data[base + 1] = palette[3 * row[x] + 1]; canvasData.data[base + 2] = palette[3 * row[x] + 2]; canvasData.data[base + 3] = 255; } } ctx.putImageData(canvasData, xc, yc); }
function draw_sprite_rlet(ctxHandle,paletteP,xc,yc,data){ const width = HEAPU8[data]; const height = HEAPU8[data + 1]; const palette = HEAPU8.subarray(paletteP, paletteP + (3 * 256)); const ctx = Emval.toValue(ctxHandle); const canvasData = ctx.getImageData(xc, yc, width, height); let p = data + 2; for (let y = 0; y < height; y++) { let x = 0; while (x < width) { const n_transparent = HEAPU8[p++]; x += n_transparent; if (x >= width) { break; } let n_opaque = HEAPU8[p++]; while (n_opaque-- > 0) { const color = HEAPU8[p++]; const d = 4 * y * width + 4 * x; canvasData.data[d] = palette[3 * color]; canvasData.data[d + 1] = palette[3 * color + 1]; canvasData.data[d + 2] = palette[3 * color + 2]; canvasData.data[d + 3] = 255; x += 1; } } } ctx.putImageData(canvasData, xc, yc); }
...
var asmLibraryArg = {
  ...
  "abort": _abort,
  "draw_char_internal": draw_char_internal,
  "draw_sprite": draw_sprite,
  "draw_sprite_rlet": draw_sprite_rlet,
